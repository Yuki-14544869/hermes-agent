...
### 5. 基础设施即代码 (IaC & Ops Scripts)
- **安全发布闭环**：
  - `safe_update.sh`: 集成远端 Fork 仓库强制同步、自动 Rebase 及无冲突下的自动 Push 闭环。
  - `safe_push.sh`: 强制仅提交已跟踪文件，隔离脏工作区与未跟踪配置。
- **定时体检**：提供 `elysia-nightly-patrol.sh` 巡检脚本，供 Cron 任务定期调用。
