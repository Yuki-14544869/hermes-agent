...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    try:
        ...
        # Notify the user immediately so they know a fallback happened,
        # even when the request ultimately succeeds (buffer would be cleared).
        agent._emit_status(f"🔄 Fallback: {old_model} → {fb_model} ({fb_provider})")
        return True
    except Exception as e:
